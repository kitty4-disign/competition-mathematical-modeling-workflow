---
name: write-himcm-paper
description: "Use only when the user explicitly requests a HiMCM / High School Mathematical Contest in Modeling paper, a paper matching 16226.docx, or HiMCM-format DOCX/PDF. Create an English, evidence-based HiMCM submission from the contest problem, verified calculations, and project result figures. Do not use for generic reports or other modeling contests."
---

# HiMCM Paper Export

Read `references/16226-format.md` before drafting. For an HPC-energy problem, also read `references/hpc-data-source-table.md`. Treat these as mandatory production rules, not suggestions.
...
4. Export the **same Markdown** through both `create_word_document` (`document_type: "modeling_paper"`) and `create_pdf_document`. Never claim an export succeeded until both tools report success.
...
- Use A4, single-column body text. Generate **no header and no footer**, including no team number, page number, date, or running title.
...
- Before delivery confirm: no header/footer; title + Summary-only first page; background/literature citation counts; Task prose is paragraph-based; every table/figure has a consecutive caption; every display equation has a following `Where` paragraph; conclusion has two paragraphs; and Appendix contains code only.
